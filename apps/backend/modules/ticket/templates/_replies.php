...
<br /><ul class="sf_admin_actions" style="padding-left: 20px;">
    <li class="sf_admin_action_new">
        <?php echo link_to('Add Reply', "ticket_comment/new?ticket_id=".$ticket->getId(), array("popup"=>array("reply", "width=600,height=500") ) );?>
    </li>    
</ul><br />
<?php else: ?>
...
<?php foreach( $ticket->getTicketComment() as $comment ): ?>
    ...
    <h3 style="padding: 10px 10px 10px 32px; "><?php echo $sender." - ".format_date($comment->getCreatedAt(), "d-M-y"); ?></h3>
    <div class="sf_admin_list ui-grid-table ui-widget ui-corner-all ui-helper-reset ui-helper-clearfix" style="min-height: 80px;">
        <?php if($comment->getRepliedBy()):?>
        ...
        <?php else:?>
            <?php echo $comment->getPublicMessage();?>
        <?php endif;?>
