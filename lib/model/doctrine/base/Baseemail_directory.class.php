<?php

/**
 * Baseemail_directory
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property varchar $directory_key
 * @property varchar $title
 * @property varchar $send_to
 * 
 * @method integer         getId()            Returns the current record's "id" value
 * @method varchar         getDirectoryKey()  Returns the current record's "directory_key" value
 * @method varchar         getTitle()         Returns the current record's "title" value
 * @method varchar         getSendTo()        Returns the current record's "send_to" value
 * @method email_directory setId()            Sets the current record's "id" value
 * @method email_directory setDirectoryKey()  Sets the current record's "directory_key" value
 * @method email_directory setTitle()         Sets the current record's "title" value
 * @method email_directory setSendTo()        Sets the current record's "send_to" value
 * 
 * @package    BestBuddies
 * @subpackage model
 * @author     Anvaya Technologies
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class Baseemail_directory extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('email_directory');
        $this->hasColumn('id', 'integer', 8, array(
             'type' => 'integer',
             'size' => 8,
             'unsigned' => true,
             'primary' => true,
             'autoincrement' => true,
             'length' => 8,
             ));
        $this->hasColumn('directory_key', 'varchar', 20, array(
             'type' => 'varchar',
             'unique' => true,
             'notnull' => true,
             'length' => 20,
             ));
        $this->hasColumn('title', 'varchar', 255, array(
             'type' => 'varchar',
             'notnull' => false,
             'length' => 255,
             ));
        $this->hasColumn('send_to', 'varchar', 4000, array(
             'type' => 'varchar',
             'notnull' => false,
             'length' => 4000,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($timestampable0);
    }
}